...
module.exports = async (sock, from, argumentos) => {
  ...
  // Log de parámetros
  console.log(":: Ficha de cereales parametros recibidos -> " + coope + " " + cuenta + " " + cereal + " " + clase + " " + cosecha + " tipo: " + tipo);
  ...
  const pdfResponse = await axios.post('https://dev.kernelinformatica.com.ar/reportes/generarReportePdf', {
    ...
    const tempPath = './pdfs/'+cuenta+'-ficha-cereales-temp.pdf';
    ...
    writer.on('finish', async () => {
      ...
      await sock.sendMessage(from, {
        ...
        fileName: cuenta+'-ficha-cereales.pdf',
      });
    ...
    writer.on('error', async (error) => {
      ...
      await sock.sendMessage(from, { text: mensajes.error_obtencion_ficha_cereales });
    });
